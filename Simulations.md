...
The waveform shows that the period of the `ce` pulse changes according
to the selected `cnt_set(2:0)` value.
...
The waveform shows ball movement through `LED(15:0)` and the edge or
overflow indication signals generated by the counter.
